...
int main(int argc, char **argv)
{
    ...
    if (argc != 15)
    {
        fprintf(stderr, "usage: ./assemble seed epsilon0 kappa0 kappaPhi0 theta0 theta1 LnK muCd ks0 dmu dg mudrug gdrug kd0\n");
        exit(-1);
    }
    ...
    for (int i = 0; i < g.Ntype; i++)
    {
        for (int j = 0; j < g.Ntype; j++)
        {
            ...
            if (i == 1 && j == 2)
                g.gb[i][j] = gb0 + (3* g.dg * gb0); // BA-AB
            else if (i == 0 && j == 1)
                g.gb[i][j] = gb0 + (1 * g.dg * gb0); // CD-AB (T3)
            else if (i == 3 && j == 1)
                g.gb[i][j] = gb0 + (1 * g.dg * gb0); // DC-AB
            else if (i == 2 && j == 0)
                g.gb[i][j] = gb0 + (-1 * g.dg * gb0); // AB-DC
            else if (i == 2 && j == 3)
                g.gb[i][j] = gb0 + (-1 * g.dg * gb0); // 
            else if (i == 3 && j == 3)
                g.gb[i][j] = gb0 + (0 * g.dg * gb0);
            else if (i == 0 && j == 0)
                g.gb[i][j] = gb0 + (-7 * g.dg * gb0);
            else if (i == 0 && j == 3)
                g.gb[i][j] = gb0 + (-7 * g.dg * gb0);
            else if (i == 3 && j == 0)
                g.gb[i][j] = gb0 + (-7 * g.dg * gb0);
    ...
    FILE *efile, *finalfile, *fi, *paramfile; // , *eefile; ,
    g.dump_parameters();

    if( access( "energy.dat", F_OK ) != -1 ) {
        fprintf(stderr," log files exist\n");
        efile = fopen("energy.dat", "a");

    } else {
        paramfile = fopen("allparam.dat","a");
        // XXX Update this 
        fprintf(paramfile, "#sweep, seed,seconds, g.epsilon[0], g.kappa[0], g.theta0[0],g.theta0[1], gb0, g.mu[0], g.mu[1], g.dg, ks0,g.theta0[2],kd0,gdrug0, g.mudrug , g.compute_energy(), g.Nd,g.Nsurf, g.Nv, NE\n");
        fclose(paramfile);
        efile = fopen("energy.dat", "a");
        fprintf(efile, "#sweep, seed,seconds, g.epsilon[0], g.kappa[0], g.theta0[0],g.theta0[1], gb0, g.mu[0], g.mu[1], g.dg, ks0,g.theta0[2],kd0,gdrug0, g.mudrug , g.compute_energy(), g.NAB, g.NCD, g.Nd,g.Nsurf, g.Nv, NE");
        
    // file doesn't exist
    }
    fi = fopen("parameters_run.out", "a");
    fprintf(fi, "./source/assemble seed epsilon0 kappa0 kappaPhi0 theta0 theta1 LnK muCD ks0 dmu mudrug gdrug kd0\n");
    fprintf(fi, "./source/assemble %lu %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.6f %.3f %.3f %.3f %.3f %.6f\n", seed, g.epsilon[0], g.kappa[0], Phikappa, g.theta0[0], g.theta0[1], gb0, g.mu[0], ks0, dmu, g.dg, g.mudrug , gdrug0, kd0);
    for (int i = 0; i < g.Ntype; i++)
    {
        for (int j = 0; j < g.Ntype; j++)
        {
    ...
    fprintf(stderr, "./source/assemble %lu %f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.6f %d\n", seed, g.epsilon[0], g.kappa[0], Phikappa, g.theta0[0], g.theta0[1], gb0, g.mu[0], ks0, g.mu[1], g.dg, g.mudrug , gdrug0, kd0, sweep);
    ...
    cout << "# NHE " << g.Nhe << " # NHESURF " << g.surfheid.size() <<" # NVSURF " << g.surfv.size() << " # NV_BONDSURF " << g.surfvbond.size() << " #" << endl;
    ...
    //for (int sw=0;sw<1001; sw++)
    {
        ...
        { //complete capsid
            ...
            g.update_surface();
            //dump_lammps_data_file(g, int(sweep/freq_vis)+1);
            //dump_lammps_data_file(g, 22222222);
            time(&timer2);
            seconds = difftime(timer2, timer1);         
            fprintf(efile, "%d %lu %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %d %d %d %d %d\n", sweep, seed,seconds, g.epsilon[0], g.kappa[0], g.theta0[0],g.theta0[1], gb0, g.mu[0], g.mu[1], g.dg, ks0,g.theta0[2],kd0,gdrug0, g.mudrug , g.compute_energy(), g.NAB, g.NCD, g.Nd,g.Nsurf, g.Nv, g.Nhe/2);
            fflush(efile);
            break;
        }
       ...
       // ADD DIMER ATTEMPT
        ...
        if (sweep % (freq_vis)==0) {
            recenter(g);
            dump_lammps_traj(g, int(sweep/freq_vis)); 
        ...
        if (sweep == 100000000)
        {
            ...
            g.update_surface();
            dump_lammps_data_file(g, 88888888);
            exit(-1);
        }
    ...
    dump_lammps_traj(g, int(sweep/freq_vis)+1);
    ...
    fprintf(finalfile, "%d %lu %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %d %d %d %d %d\n", sweep, seed,seconds, g.epsilon[0], g.kappa[0], g.theta0[0],g.theta0[1], gb0, g.mu[0], g.mu[1], g.dg, ks0,g.theta0[2],kd0,gdrug0, g.mudrug , g.compute_energy(), g.NAB, g.NCD, g.Nd,g.Nsurf, g.Nv, g.Nhe/2);
